...
class Marketing extends CI_Controller {
 ...
 function quotation_process_v2(){
  ...
  //       if ($svalue <> $bulanini) {$skey = 1;} //Jika bulannya berbeda maka harus diubah mejadi satu..
		...
		$gabung 		= $name_client{0}."".str_pad($id_client, 4, "0", STR_PAD_LEFT)."/".$package_name."/".$kodeq."/".$urutan;
 	...
 	if($this->session->userdata('logged_in')){
		...
		if ($jmldoubledata > 0) {
			...
			for ($i=1; $i <=$rowcount ; $i++) { 
				...
				if ($group[$i] == "") {$group[$i]="Administration";}
				if ($orderty[$i] == "") {$orderty[$i]=4;}
				if ($orderid[$i] == "") {$orderid[$i]=0;}
				if ($id_service[$i] == "") {$id_service[$i]=0;}
				if ($seq[$i] == "") {$seq[$i]=0;}
...
function list_service_price_process(){
 ...
 function process_update_mst_service(){
