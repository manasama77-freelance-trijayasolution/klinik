...
class Cashier extends CI_Controller {
 ...
 function counter(){
	...
	function input_billing_process(){
	    ...
	    if($this->session->userdata('logged_in')){	
			...
			// Mendapatkan kurs paling update....
				...
				if ($cambilkurs > 0) {
					foreach ($ambilkurs->result() as $row) {$nk = $row->amount;}
					$nilaikurs = $nk;
				}else{
					foreach ($kursakhir->result() as $row) {$nka = $row->amount;}
					$nilaikurs = $nka;
				}
			...
			if ($jml == 0) { 
				...
				foreach ($max_billing->result() as $rowb) {$data['id_billing']= $rowb->id_billing;}
	...
	function print_invoice_all(){
 	...
 	function save_payment(){
 		if($this->session->userdata('logged_in')){	
			...
			$get_payment_header		= $this->m_cashier->get_max_trx_pat_payment_h($id_reg);	
			foreach ($get_payment_header->result() as $row) {
  ...
  function report_patient_excel(){
		...
		$datereg1 					= date("Y-m-d", strtotime($this->input->post('datereg1')));
		$datereg2 					= date("Y-m-d", strtotime($this->input->post('datereg2')));
		$data['datereg1']			= date("d-m-Y", strtotime($this->input->post('datereg1')));
		$data['datereg2']			= date("d-m-Y", strtotime($this->input->post('datereg2')));
