 ...
 function save_order_lab(){
	...
	function list_report_mcu_detail(){
		...
		// -------------- memasukan data ke temporary table -----------------
			...
			if (isset($tgl_reg[1])){$data['reg_date_previous'] = date("d-m-Y", strtotime($tgl_reg[1]));}
			if (isset($tgl_reg[2])){$data['reg_date_last'] = date("d-m-Y", strtotime($tgl_reg[2]));}
			...
			// for($x = 0; $x < 10; $x++) {
				...
				if (empty($jml_cek)) { // --- Jika data belum ada maka insert ---
					...
					foreach ($detaillab->result() as $v_lab) {
						...
						if ($content_h != "") {$content_h = $content_d;}
			...
			foreach ($cek_header_table->result() as $row) {$data['arr_service_other'][] 	= $row->content_d;}
