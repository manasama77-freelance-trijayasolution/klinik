				<script type="text/javascript">
					...
					window.opener.document.forms['mark_mcu'].elements['gender'].value=myarr[7];
					window.opener.document.forms['mark_mcu'].elements['pat_dob'].value=myarr[8];
					window.close(this);
					}
				</script>
				<?php
  									...
  									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
										...
										?>
											<tr class="odd gradeX">
												...
												?>
												<a href="" onclick="closeit('<?=$row->id_reg;?> - <?=date("d.m.Y",strtotime($row->reg_date));?>:<?=$row->pat_name;?>, <?=$row->title_desc;?>:<?=$row->id_reg;?>:<?=findage_detail($row->pat_dob);?>:<?=$row->package_name;?>:<?=$row->client_name;?>:<?=$row->id_Pat;?>:<?php echo $row->gender;?>:<?php echo $row->pat_dob;?>');"><button type="submit" class="btn btn-info btn-mini"><i class="icon-plus-sign"></i></button></a>
												<?php
												}else{
												?>
												<button class="btn btn-danger btn-mini tooltip-top" data-original-title="Locked by <?php echo $row->fullname;?>"><i class="icon-lock"></i></button>
												<?php
												}
												?>
												</td>
