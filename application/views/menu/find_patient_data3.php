  									...
  									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
										...
										?>
											<tr class="odd gradeX">
												...
												<td><a href="" onclick="closeit('<?=$row->pat_name;?>:<?=$row->id_Pat;?>:<?=$row->noreg;?>:<?php echo $row->pat_dob;?> / <?php echo $row->pat_pob;?> / Age <?=findage_detail($row->pat_dob);?>:<?=$row->client_name?>:<?=$row->id_client?>:<?=$row->package_name?>:<?=$row->id_package?>:<?=$row->reg_date?>');"><button type="submit" class="btn btn-mini"><i class="icon-plus-sign"></i></button></a></td>
											</tr>
										</form>
										<?php
										}
