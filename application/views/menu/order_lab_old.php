	...
	<script>
	  ...
	  function popup(b_id){
		window.open("<?php echo base_url();?>patient/find_patient","Popup","height=550, width=1025, top=70, left=180 ");
      }
	  ...
	  function btntest_onclick(){
                                      ...
                                      <fieldset>									 									 
										...
										<script language="javascript">
											function addRow_lab(tableID){
												...
												cell3.innerHTML = "<input type='text' placeholder='start typing here. . .' onclick='if(this.value!=\"\") this.value=\"\";' onblur='javascript:if(this.value==\"\"){this.value=this.value;}' style='width: 350px;font-style: oblique;' class='span6' id='typeahead' name='lab_"+cell2.innerHTML+"' data-provide='typeahead' data-items='15' data-source='[<?php foreach($data->result() as $row){ echo '\"'.$row->id_lab_item.":[GROUP] ".$row->group_name.":[ITEM] ".$row->lab_item_desc.'\",'; }?> \"\"]\' autocomplete='off' required><input type='hidden' name='rowC' value='"+cell2.innerHTML+"'>";
											}
		...
		<table class="table table-striped table-bordered" id="example4">
			...
			<tr class="odd gradeX" id="voucher_">
				<TD>1</TD>
				<TD><input type="text" placeholder="start typing here. . ." onclick="if(this.value!='') this.value='';" onblur="javascript: if(this.value==''){this.value=this.value;}" style="width: 350px;font-style: oblique;" class="span6" id="typeahead" name="lab_1" data-provide="typeahead" data-items="15" data-source='[<?php foreach($data->result() as $row){ echo '"'.$row->id_lab_item.":[GROUP] ".$row->group_name.":[ITEM] ".$row->lab_item_desc.'",'; }?>""]' autocomplete="off" required></TD>
