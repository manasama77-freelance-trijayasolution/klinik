  									...
  									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
										...
										?>
											<tr class="odd gradeX">
												...
												?>
												<a href="" onclick="closeit('<?=$row->id_reg;?> - <?=date("d.m.Y",strtotime($row->reg_date));?>:<?=$row->pat_name;?>, <?=$row->title_desc;?>:<?=$row->id_reg;?>:<?=findage_detail($row->pat_dob);?>:<?=$row->package_name;?>:<?=$row->client_name;?>:<?=$row->id_Pat;?>:<?=$row->reg_date;?>');"><button type="submit" class="btn btn-info btn-mini"><i class="icon-plus-sign"></i></button></a>
												<?php
												}else{
												?>
