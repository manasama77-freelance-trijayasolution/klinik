...
<script>
	...
	function print_receipt() {
		...
		// window.open("<?php echo base_url(); ?>cashier/print_receipt/<?php echo $id_bh; ?>/<?php echo $id_billing; ?>", "", "width=700, height=550");
	}
</script>
<?php
...
$totbayar			= 0;
$jumlahpay			= $payment->num_rows();
if ($jumlahpay > 0) {
	foreach ($payment->result() as $row_pay) {
		$tagihan 	= $tagihan - $row_pay->am2;
		$totbayar 	= $tagihan - $row_pay->am1;
	}
}
?>
<!-- morris stacked chart -->
<div class="row-fluid" onmouseover="hitjumlah(<?= $tagihan; ?>);" id="notification-sticky">
	<!-- block -->
	<div class="block">
							...
							<div style="width:50%; float:left;">
								...
								<div class="control-group">
									...
									<div class="controls">
										...
										<input name="id_bh" type="hidden" value="<?= $id_bh; ?>" autocomplete="off">
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input class="input-xlarge datepicker" name="pay_date" id="pay_date" value="<?= date("m/d/Y", strtotime("now")) ?>" type="text" autocomplete="off" required=""> <i class="icon-calendar"></i>
									</div>
								</div>
								...
								?>
									<div class="control-group">
										<label class="control-label" for="focusedInput">Convertion</label>
										<div class="controls">
...
-->
								...
								<input type="hidden" name="kurs" id="kurs" value="<?= number_format($row_kurs->amount, 2); ?>">
								<input type="hidden" name="kursx" id="kursx" value="<?= $row_kurs->amount; ?>">
								<input type="hidden" name="usdy" id="usdy" value="0">
								<input type="hidden" name="usd" id="usd" value="0">
								<input type="hidden" name="usdx" id="usdx" value="0">
								...
								<tr class="odd gradeX">
									...
									<td>Credit Card</td>
									<td>
										<select class="chzn-select" name="bank_cc" id="bank_cc" style="width:350px;">
								...
								<tr class="odd gradeX">
									...
									<td>Debit Card</td>
									<td>
										<select class="chzn-select" name="bank_dc" id="bank_dc" style="width:350px;">
											<option value="0">- Choose Bank -</option>
											<?php
											foreach ($bank->result() as $rows) {
											?>
									...
									<td>
										<div align="right">
											...
											<input class="input-xlarge-in focused" name="dcx" id="dcx" value="0" style="width:145px; text-align:right" type="text">
										</div>
									</td>
								...
								<tr class="odd gradeX" style="display: none;">
									...
									<td>
										<select name="bank_ins" id="bank_ins" style="width:350px; display: none;">
											<option value="0">- Choose Insurance -</option>
											<?php
											foreach ($insurance->result() as $rows) {
											?>
												<option align="justify" value="<?= $rows->id_ins_comp ?>"><?= $rows->ins_name ?></option>
											<?php
											}
											?>
										</select>
									...
									<td>
										<div align="right">
											<input class="input-xlarge-in focused" name="noins" id="noins" placeholder="Polis Number" style="width:145px; text-align:right; display: none;" type="text" autocomplete="off" <?= $readonly_ins; ?> maxlength="10">
										</div>
									</td>
									<td>
										<div align="right">
											<input class="input-xlarge-in focused" name="ins" id="ins" value="0" style="width:145px; text-align:right; display: none;" type="text" autocomplete="off" onchange="hitungan(); hitjumlah(<?= $tagihan; ?>);" onclick="if(this.value==0) this.value='';" onblur="javascript: if(this.value==''){this.value=0;}" <?= $readonly_ins; ?>>
						...
						<table class="table table-bordered">
							<tbody>
								...
								<tr class="odd gradeX">
									<td>
										<div align="right"><b>Bill</b> <input class="input-xlarge-in focused" id="bill" name="bill" value="<?= number_format($tagihan, 2); ?>" style="width:145px; text-align:right" type="text" autocomplete="off" readonly="on"> <input class="input-xlarge-in focused" id="billx" name="billx" value="<?= $tagihan - $totbayar; ?>" style="width:145px; text-align:right" type="hidden" readonly="on">
										</div>
									</td>
								</tr>
								<tr class="odd gradeX">
...
<input type="hidden" id="num_payment_before" value="<?= $payment_before->num_rows(); ?>" />
<?php
function payment_before_growl($payment_before)
{
	$aa = 1;
	foreach ($payment_before->result() as $row_last_prec) {
		echo $aa++ . ". " . number_format($row_last_prec->amount, 2) . ", <span style='font-size: 85%'> " . time_elapsed_string1($row_last_prec->create_date) . "</span></br>";
	}
}
?>
<script>
	if ($('#num_payment_before').val() > 0) {
		window.onload = $.jGrowl(`
		<b>Last Transaction</b> : </br> <?php payment_before_growl($payment_before); ?>`, {
			sticky: true
		});
	}
</script>