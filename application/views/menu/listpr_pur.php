				...
				function delete_order(group,id) {
					var myWindow = window.open("<?php echo base_url();?>master/add_notes/"+group+"/"+id+"", "Popup", "width=1025, height=600, top=50, left=180");
				}
				
				function view_notes(group,id) {
					var myWindow = window.open("<?php echo base_url();?>master/view_notes/"+group+"/"+id+"", "Popup", "width=1025, height=600, top=50, left=180");
				}
                     ...
                     <div class="row-fluid">
                        ...
                        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left"><b>List Purchase Request</b></div>
                                   ...
                                   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example2">
										...
										?>
                                            <tr class="odd gradeX">
												...
												<td><?=$row->fullname;?></td>
                                                <td><?=$row->nama_dep;?></td>
												<!-- <td><?=str_replace(";"," <br> ",$row->items);?></td> -->
												<td style="text-align: center;">
													
													<button onclick="view_items(<?=$row->id_pr_no;?>)" title="Close" class="btn btn-info btn-mini"><i class="icon-folder-open"></i></button>

													<button class="btn btn-success btn-mini" title="Print" onclick="myFunction_3(<?=$row->id_pr_no;?>)"> <i class="icon-print"></i> </button>

													<?php if ($row->is_finalized == 5) { ?>											
														<button class="btn btn-warning btn-mini" onclick="view_notes(5,<?=$row->id_pr_no;?>)"> <i class="icon-comment"></i> </button>
													<?php }else{ ?>
														<button onclick="delete_order(5,<?=$row->id_pr_no;?>)" title="Close" class="btn btn-danger btn-mini"><i class="icon-trash"></i></button>
													<?php } ?>
